#!/bin/bash
#
# Copyright Dixing Xu 2018 All Rights Reserved
#
...
function printHelp () {
  echo "Usage: "
  echo "  3org.sh up|down|restart|generate [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>]"
  echo "  3org.sh -h|--help (print this message)"
  ...
  echo
  echo "	3org.sh generate -c mychannel"
  echo "	3org.sh up -c mychannel -s couchdb"
  echo "	3org.sh up -l node"
  echo "	3org.sh down -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	3org.sh generate"
  echo "	3org.sh up"
  echo "	3org.sh down"
}
...
function networkUp () {
  ...
  if [ ! -d "artifacts/org3-artifacts/crypto-config" ]; then
    generateCerts
...
function networkDown () {
  docker-compose -f artifacts/$COMPOSE_FILE -f artifacts/$COMPOSE_FILE_ORG3 -f artifacts/$COMPOSE_FILE_COUCH down --volumes --remove-orphans
...
function createConfigTx () {
  ...
  echo "###############################################################"
  docker exec -it cli scripts/step1org3.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
